...
| `sistemas` | 🔴 ocupada | 1d765cd1 — **npm test quebrado no PC: `deOutraPlataforma()` recusa qualquer caminho `C:\` como "de outra máquina", inclusive rodando no próprio Windows.** `acharRoadmap` não achava nem o ROADMAP.md deste projeto, cwd real, sem federação no meio — roadmap ficava cego no PC inteiro. Fix de duas linhas: só recusa quando NÃO está no Windows. Peguei só `src/roadmap.mjs`; o resto que a `sistemas` reservava (`cc.mjs`, `src/meu.mjs`, `src/sessoes.mjs`, `src/tarefasProtocolo.mjs`, `src/hooksCatalogo.mjs`, `src/config.mjs`, `src/routia.mjs`, `src/testmap.mjs`, `src/armazem.mjs`, `src/coletores.mjs`, `test.mjs`, `test-map-vivo.mjs`, `package.json`, hooks de tarefa) continua livre **Também peguei `test.mjs`: o teste CC-280 de pasta trancada assume que `chmod 0500` bloqueia escrita — no Windows não bloqueia (testei de verdade, escrita passou igual), então o teste pula lá, como já acontece com o Pierre e os extratores.** **`src/install.mjs` (emprestado da `front`) e `test-quebra.mjs` (emprestado da `quebra`) DEVOLVIDOS: `projectsBases()` quebrava caminho do Windows nas letras de unidade (corrigido, separa por `;` quando `ehWindows`), e `test-quebra.mjs` pula na máquina sem `~/projetos`, mesmo padrão do Pierre.** **Fechado também: CC-351, o serviço supervisionado do PC.** `src/bandeja.ps1` (novo, o ícone na barra com o traço entre dois nós, provado ao vivo, ele confirmou o desenho) e `src/arrancar.ps1` (novo, o lançador único: git pull, sobe painel e bandeja, vida presa à do painel). A Tarefa Agendada agora chama esse lançador em vez do painel direto. **Achado que não é bug: criar tarefa agendada nesta máquina exige PowerShell como administrador, mesmo pra tarefa simples sem elevação nenhuma no XML — é do Windows dele, não do código.** Ele já instalou de verdade (`AgentCockpit`, modo boot) e confirmou visualmente o ícone. Falta só a prova de sobreviver a dormir/deslogar, que só dá pra fazer na próxima pausa natural dele. **Investigado, não fechado: os servidores do PC chegam na VPS de forma instável.** Achei uma SEGUNDA fonte escrevendo no pacote da máquina, que eu não sou, com carimbo de hora que não bate com o meu — ticket completo no topo desta seção. `src/web.mjs` foi emprestado só pra instrumentar (log temporário, DEVOLVIDO ao fechar, arquivo voltou pro estado do git nos dois lados, PC e VPS religados limpos). Meu processo sozinho está correto e provado; a segunda fonte continua desconhecida. 🎚 restritivo 📁 src/roadmap.mjs 📁 test.mjs | hoje |
| `front` | 🔴 ocupada | d4b47d4e — **TOMADA de fbabdeb0 em 25/08, com autorização dele na hora ("quero ver as sessões separadas na central").** Medido antes: fbabdeb0 está parada há 96 min (último sinal 22:07) e `src/ui_v2.html` está LIMPO, sem nada dela sem salvar. **CC-356 (sessões separadas na Central):** o cartão junta `VPS_cockpit` e `VPS_cockpit-2` num controle só, e o "conectar celular" sempre pega a primeira. Confirmado no painel vivo: dois `cc-remote-*` ativos. Desenho uma linha por rótulo em `acoesDeSessao`, cada uma com seu conectar/soltar/encerrar; o backend já age por rótulo. Se fbabdeb0 voltar, **o CC-343 continua sendo dela** (a explicação do modo embaixo do seletor), e nada dela foi tocado. **CC-357, na sequência:** os rótulos do controle remoto (`VPS_cockpit`, `VPS_cockpit-2`) apontavam todos para a conversa mais nova, medido no painel vivo. Caso cada um à conversa que nasceu logo depois dele (correlação de tempo de criação), pré-requisito do modo por sessão. **CC-358: o modo por sessão de fato** (o pedido dele). `src/web.mjs` EMPRESTADO em 26/08, com autorização dele: medido que e859ce91 (escritório) está parada há 31h e fbabdeb0 (sincronia) há 4h, e o arquivo está limpo. Só ACRESCENTO: cada sessão reporta o modo dela em `/api/remote-control`, e a ação `modo` aceita `sessao` para gravar a capa por sessão. **`src/web.mjs` cedido pra `sistemas` em 26/08, com autorização dele na hora: 1d765cd1 vai instrumentar temporariamente pra achar um bug de servidores instáveis.** Devolvo ao fechar 🎚 continuativo 📁 src/ui_v2.html 📁 test-central-sessoes.mjs 📁 src/remotecontrol.mjs 📁 test-remoto-conversa.mjs | hoje |
