...
| `sincronia` | 🟢 livre | — **84be1862 fechou em 10/09: os dois lançadores duplicados (CC-456) e o loop de bandeja que ele viu piscando (CC-459), com a causa e o conserto do CC-459 herdados de `ae131ce9` (que tinha o pedido de rota travado esperando esta liberar). Achado no caminho: `cc daemon restart`/`cc open` recriavam sozinhos o `.vbs` sem supervisão sempre que ele não existisse, reabrindo o CC-456 a cada restart. Corrigido em `subirDestacado()`: prefere a Tarefa Agendada quando ela já existe. Provado na máquina: painel de pé, sob supervisão real, contagem de powershell estável por 30s. Gate verde** 📁 src/daemon.mjs 📁 src/arrancar.ps1 📁 src/platform.mjs | 2026-09-10 |
