...
| `sistemas` | 🔴 ocupada | 721fa1f4 — **o gate não roda no Windows depois do pull de 26/08, e para no PRIMEIRO teste.** `test-anonimizar.mjs` importava por caminho (`file://D:\...`), que o Node recusa no Windows (`ERR_UNSUPPORTED_ESM_URL_SCHEME`), então os outros 13 testes nem chegavam a rodar. Corrigido com `pathToFileURL` nos dois pontos do arquivo; gate volta a passar (171 ok, 3 pulados, exit 0). **Mesmo defeito existe em `hooks/commit-auto.mjs:49`, e lá é CALADO**: a guarda "fui chamado direto?" compara `import.meta.url` com `file://${process.argv[1]}`, que nunca casa no Windows, então o gancho carrega e não faz nada. Esse arquivo é da rota `commit-auto` (d4b47d4e, que NÃO existe nesta máquina e cujo arquivo está limpo aqui); **`hooks/commit-auto.mjs` EMPRESTADO e DEVOLVIDO em 26/08, com autorização dele por escrito no chat ("libera"), gravada com motivo**: a rota `commit-auto` (d4b47d4e) não existe nesta máquina e o arquivo estava limpo aqui. Só a guarda de "fui chamado direto?" foi trocada por `pathToFileURL`; o resto do arquivo não mudou. Provado antes e depois: chamado direto no Windows saía calado com código 0, agora responde o estado. Gate: 171 ok, 3 pulados, exit 0. **📌 `src/ui_v2.html` EMPRESTADO da rota `front` em 26/08, com autorização dele na hora, para o CC-361.** Medido antes: a dona d4b47d4e NÃO existe nesta máquina e o arquivo está LIMPO aqui. Só ACRESCENTO o bloco de liberar escrita (`fw-aut` / `fw-pedidos`) e o `alvo` no tratador de clique que já existe; nada do CC-356, CC-357 nem CC-358 é tocado. Devolvo ao fechar. **Achado que motivou, CC-361** — o botão de liberar escrita só existe em `src/ui.html` (`/v1`), e não na tela padrão (`ui_v2.html`), então a trava barra sem ter onde clicar 🎚 continuativo 📁 test-anonimizar.mjs 📁 test.mjs | hoje |
