...
| `sistemas` | 🟢 livre | — **LIBERADA por e859ce91 em 24/08, com autorização dele na hora.** 9bad715c parou de escrever em 22/08 às 09:23 (calada há 57h) e não havia processo vivo. Ela cuidava do que roda por baixo: protocolo, hooks, comandos e o gate, com o CC-232 fechado. **`src/web.mjs` saiu dela ainda antes, em 24/08, para o conserto do vazamento de memória (CC-333), e está na rota `escritorio`.** Os outros arquivos que ela reservava voltam a ficar livres: `cc.mjs`, `src/meu.mjs`, `src/sessoes.mjs`, `src/tarefasProtocolo.mjs`, `src/hooksCatalogo.mjs`, `src/config.mjs`, `src/routia.mjs`, `src/roadmap.mjs`, `src/testmap.mjs`, `src/armazem.mjs`, `src/coletores.mjs`, `test.mjs`, `test-map-vivo.mjs`, `package.json` e os hooks de tarefa | — |
| `front` | 🔴 ocupada | 2681f3b6 — **CC-332, pedido dele em 23/08 olhando o telefone:** a central de comando não abre sessão nem conversa. Clicar em "abrir sessão" num projeto que já tem sessão devolve a que existe e não avisa nada (medido: a rota responde `ja:true`), o cartão não muda quando há sessão no ar, e não há caminho nenhum para o Coderoom nem para abrir uma segunda de cada. Dona de `src/ui_v2.html`, retomado da rota `gate` em 23/08 com autorização dele, depois de medir que c213b663 está órfã há 32h. **📌 `src/ui_v2.html` foi EMPRESTADO à rota `remote-control` (21e88ed9) em 24/08 às 20:20, com autorização dele na hora**, só para acrescentar os botões de conectar, soltar e reabrir na tela de sessões. Medido antes: 2681f3b6 não tem transcrito nesta máquina, então a dona ou está no PC dele ou não existe mais. O CC-332 continua sendo dela 🎚 continuativo | hoje |
...
| `remote-control` | 🔴 ocupada | 21e88ed9 — **os quatro botões, e o caminho de volta.** Nasceu de um estrago meu em 24/08: li a hora de CRIAÇÃO da sessão como se fosse a da última atividade, chamei de parada há 15 horas uma conversa que tinha respondido 3 minutos antes, recomendei a ele matar as duas, e ele perdeu o acesso ao trabalho em curso do coepiloto (recuperado depois com `--resume`, nada se perdeu em disco). Medido em sessão de teste: uma conversa nasce sem acesso remoto e ganha depois com `/rc`; desconectar o remoto NÃO mata a conversa. **Correção do que eu tinha escrito antes:** o endereço NÃO volta idêntico. Medido em três ciclos, soltar de verdade faz o endereço seguinte ser outro, e ele só se repete quando a desconexão não aconteceu. Por isso o botão devolve sempre o endereço de agora, e nada guarda link antigo. Então abrir, conectar o celular, soltar o celular e encerrar viram quatro ações separadas, e encerrar passa a guardar qual conversa estava ali para poder reabrir de onde parou. Decisão dele em 24/08, nesse desenho. **`src/web.mjs` e `src/ui_v2.html` ENTRARAM aqui em 24/08, com autorização dele na hora**, só para acrescentar: a rota nova das três ações e os botões. Medido antes de pedir: 2681f3b6 (dona de `src/ui_v2.html`) não tem transcrito nesta máquina, e e859ce91 (dona de `src/web.mjs`) está calada desde 18:20. Nada do que elas fazem é alterado, só acrescento trecho novo 📁 src/remotecontrol.mjs 📁 src/web.mjs 📁 src/ui_v2.html | hoje |
