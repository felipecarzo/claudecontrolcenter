...
| `bancada` | 🔴 ocupada | d4b47d4e — **CC-354, as duas sondas de segurança do vídeo do Deyvin.** Em ordem crescente: a falha 2 (virar admin mexendo no `localStorage`) primeiro, depois a falha 3 (IDOR). Alvo escolhido por ele: fibraessencia. Só ACRESCENTO camadas novas no fim de `CAMADAS`, sem tocar nas 19 que existem 🎚 continuativo 📁 src/bancadaCatalogo.mjs | hoje |
...
| `front` | 🔴 ocupada | fbabdeb0 — **CC-343, apontado por ele em 25/08:** *"o `?` não explica os modos do framework"*. Medido: todo modo TEM explicação escrita e boa no catálogo, e ela só vive no `title` do `<option>`, que no telefone não existe. Não há verbete "framework" no glossário, então o `?` daquele ponto nem botão vira. Escrevo a explicação do modo escolhido embaixo do seletor, nos dois cartões 🎚 continuativo 📁 src/ui_v2.html | hoje |
