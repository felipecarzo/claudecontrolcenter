...
| `sincronia` | 🔴 ocupada | 5e7441d7 — **PEGA em 02/09, com autorização dele na hora ("só o conserto do dado"), depois de medir que a rota estava 🟢 LIVRE nas DUAS linhas que reivindicavam este arquivo (`sincronia`, liberada em 30/08, e `framework`, fechada em 30/08 à noite): não foi tomada de ninguém. Trabalho: **lista vazia deixa de apagar o mapa herdado** (CC-440, passo 2). `gravarPacote` herda testando `== null`, e `validarPacote` convertia `null` em `[]` em `backlogs` e `rotas`, então pacote em que a varredura falhou na outra ponta APAGAVA o roadmap inteiro que já tinha chegado, calado. Medido lado a lado com `tempo`, que herda certo. **SÓ ACRÉSCIMO de guarda**, no mesmo formato que `servidores` já usa duas linhas acima. A metade de TELA (mostrar o mapa que chega, hoje invisível) e a metade do REMETENTE (`src/web.mjs`, distinguir "varredura falhou" de "não há projeto") ficam registradas, não tomadas 📁 src/federacao.mjs 📁 test-federacao.mjs | 2026-09-02 |
