...
| `sincronia` | 🟢 livre | — **84be1862 fechou em 10/09: os dois lançadores duplicados (CC-456), o loop de bandeja (CC-459, causa e conserto herdados de `ae131ce9`), o `.vbs` sendo recriado pelo `cc daemon restart` (achado no caminho), e o merge com o commit da VPS (`758b333`), que acusou `estadoServico` e `estadoServicoAsync` discordando na mesma máquina — a VPS tinha corrigido só a versão assíncrona pra perguntar pelas duas tarefas agendadas. Espelhado no síncrono. Gate verde, provado na máquina: painel de pé, sob supervisão real** 📁 src/daemon.mjs 📁 src/arrancar.ps1 📁 src/platform.mjs | 2026-09-10 |
