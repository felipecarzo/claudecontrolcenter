...
| `framework` | 🔴 ocupada | 2d4e7b74 (PC) — **trazendo de volta o trabalho de 30/08 do PC, depois de unificar as versões.** Aquela sessão trabalhou o dia inteiro numa cópia 46 commits atrás desta, e a conferência só aconteceu quando ele mandou pôr tudo na mesma versão. O que ela fez de duplicado (o CC-362 e o sexto ponto do guia) foi DESCARTADO: a versão daqui achou três causas contra uma, e a que faltou é a mais grave. O que volta, e não existe aqui: **a trava do framework alcançando código na raiz** (`SEMPRE_LIVRE` ainda tem `'*'`, então projeto de app único nunca travou nada), **a entrevista ligada aos dois hooks** (que hoje mandam preencher o MVP à mão), **`cc pastas` mais o item na bandeja e o instalador perguntando** (a escrita e a tela já existem aqui desde 27/08, só faltam esses três), e **o retrato do framework saindo da lista de projetos** (medido no PC: 12 projetos com framework, 1 chegando na VPS). O commit guardado é `8a9c208`, na marca `salvo/pc-30-08` **Segue em 30/08 com o CC-440, na ordem que ele aprovou junto com o desenho (`docs/produto/COLETOR.md`): (1) o contrato versionado entre PC e VPS, (2) o pacote ficar rico, com o roadmap inteiro em vez de contagem e seis títulos.** `src/federacao.mjs` entra aqui pela MESMA autorização dele de hoje ("libera a rota inteira, ela está parada"), dada quando a `sincronia` foi medida como parada: a linha dela acima é do estado do remoto, anterior à autorização. Medido de novo antes de encostar: o arquivo está limpo e o último commit nele é de 25/08 📁 src/framework.mjs 📁 src/frameworkDisco.mjs 📁 src/install.mjs 📁 src/config.mjs 📁 hooks/framework-inicio.mjs 📁 hooks/framework-guard.mjs 📁 test-framework.mjs 📁 test.mjs 📁 cc.mjs 📁 src/travasDaMaquina.mjs **📌 `src/web.mjs` EMPRESTADO da `front` em 30/08 ("4. quero"), para o lado que EXECUTA o pedido de MVP vindo de outra máquina. Só um ramo novo em `atenderPedidos`. Devolvo ao fechar.** **📌 `src/federacao.mjs` EMPRESTADO da `front` em 30/08, com autorização dele na hora ("pode seguir"), para fechar a metade de recebimento do CC-445. Só acréscimo de quatro campos no `validarPacote`; a ação `recado` e o `nomeDeProjetoSeguro` que ela escreveu não são tocados. Devolvo ao fechar.** 📁 src/bandeja.ps1 📁 src/federacao.mjs 📁 src/roadmap.mjs 📁 src/platform.mjs 📁 src/publicar.mjs 📁 test-publicar.mjs 📁 test-federacao.mjs | hoje |
