...
|---|---|---|---|
| `agy-framework` | 🟢 livre | 08719863 (agy) fechou: **integração do Antigravity no framework e no Cockpit completa e provada com npm test verde.** AGENTS.md universalizado com caminhos para Linux/Agy, regra para ask_question formalizada, e transcritoDe/acharSessaoPorPrefixo aprendem a ler sessões do brain do agy no cc set. | hoje |
...
| `remote-control` | 🟢 livre | — **DEVOLVIDA por 47373acb em 22/09, trabalho fechado e gate verde.** Entrou só `binarioClaude()` em `src/remotecontrol.mjs` e as duas chamadas que a usam (tmux no Linux, `cmd` no Windows). Provado: a função devolve `/home/claudedev/.local/bin/claude` (2.1.280) e deixa passar direto qualquer outro nome, como `opencode`. Painel religado pelo `POST /api/shutdown`, PID trocou de 623958 para 1205928. **📌 `docs/backlog.jsonl` e `docs/ROADMAP.md` (da rota `simples`, parada desde 10/09) foram TOCADOS sem pedir: registrar item de backlog é o protocolo do projeto e escreve neles, e o gate exige o roadmap regerado logo depois. Três itens novos: CC-547, CC-548, CC-549. Nada do conteúdo dela foi alterado.** — **Antes, o que motivou:** **o painel sobe a versão VELHA do Claude Code depois que ele atualiza sem senha de administrador.** Medido em 22/09: o `npm` global desta VPS pertence ao root e o `sudo` do `claudedev` é recusado por política, então a via de atualização é `claude install` (instalação na pasta do usuário, `~/.local/bin`, hoje 2.1.280 contra 2.1.231 do sistema). O PATH de um serviço não é o PATH do login: o painel enxerga só as pastas do sistema, então o terminal dele usa a versão nova e toda sessão aberta PELO PAINEL continua subindo a antiga, sem erro nenhum na tela. Só acréscimo em `src/remotecontrol.mjs`: uma função que prefere a instalação do usuário quando ela existe, e o PATH como antes 🎚 continuativo 📁 src/remotecontrol.mjs — **Antes: DEVOLVIDA por d155c1cc em 21/09, CC-546 fechado com prova.** A sessão remota que nasce deslogada agora diz isso no botão. Medido em 21/09 na VPS: o login do Claude Code de lá venceu em 20/09 às 17:08, a credencial foi zerada às 19:35 de 21/09 ao abrir sessão, e toda sessão desde então nasce com "Not logged in". O painel dizia "ligado", e "conectar celular" devolvia "não achei o endereço", sem a causa. Conserto só em `src/remotecontrol.mjs` (ler a tela e devolver o motivo) mais um teste puro novo. — **Antes: 21e88ed9 fechou em 25/08.** Nasceu de um estrago meu: li a hora de criação da sessão como se fosse a da última atividade e recomendei a ele matar duas conversas, uma delas em pleno trabalho (recuperada com `--resume`). Fechados: as **quatro ações** do controle remoto com caminho de volta ao encerrar, o **CC-334** (o framework tinha dois interruptores), o **CC-336** (o cartão desentortou e o criar projeto subiu para a barra) e o **CC-337** (sessão ociosa posava de trabalhando). A **fusão de Central e Projetos** (CC-335) está no ar na primeira fatia, com o resto registrado em `docs/produto/CENTRAL-E-PROJETOS.md`. **Dois abertos que nasceram aqui e são de quem pegar a rota:** CC-338 (a rolagem que ele diz travar no PC, e que eu não reproduzi em oito cenários) e CC-339 (o cartão agrupa por dado que congela). `src/web.mjs` volta para a rota `escritorio` e `src/ui_v2.html` para a `front`, de onde vieram emprestados com autorização dele. Sete commits, todos empurrados, o último `7334998` | — |
