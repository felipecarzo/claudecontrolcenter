...
## Segunda metade do dia: os gates, as oficinas e o merge

O texto acima foi escrito às 5h da manhã. O dia continuou, e a segunda metade
mudou mais coisa que a primeira.

### A pergunta que abriu tudo: "qual hook garante a revisão e o teste?"

A resposta honesta era **nenhum**. Havia onze hooks, e nenhum verificava se o
gate tinha rodado. Pior: o `roadmap-guard` estava **mudo desde que nasceu** —
registrado no `settings.json`, mas fora do `hooksCatalogo`, e `hookEnabled`
devolve `false` para id desconhecido. Ele tinha 37 itens concluídos para acusar
no ROADMAP e nunca falou. Terceira vez que essa armadilha me pega em dois dias,
então o gate passou a acusar hook que consulta o catálogo sem estar nele.

Seis hooks novos entraram no dia, todos com teste:

| hook | garante |
|---|---|
| `fluxo-guard` | no restritivo, **parar** com backlog aberto precisa ser declarado |
| `gate-guard` | `npm test` rodou DEPOIS da última edição de código |
| `bancada-guard` | a tarefa se auto-verificou no nível do projeto |
| `edicao-guard` | nada de `sed -i` em arquivo do repo — o `Edit` recusa em voz alta |
| `guia-guard` | guia de 3+ passos vira etapa, com âncora absoluta |
| `branch-guard` | comando git que apaga trabalho de quem não está olhando |
| `anonimo-prompt` | dado pessoal colado no chat não sai da máquina |

### O `fluxo-guard` me devolveu quatro vezes, e nas quatro ele estava certo

Duas por eu ter parado anunciando continuidade. Uma por cabeçalhos de
arquivamento que pareciam itens abertos. E uma por contar como "aberto" seis
itens que eram direção ou dependiam dele — aí o errado era o hook, e o conserto
foi o marcador `⏸` com o motivo no próprio título. **Guarda que cobra o
impossível ensina a ser ignorado**, e aí não segura mais o caso real.

### O segundo agente: worktree, e o buraco que a rota tinha

Ele quis abrir um agente para o front-end e perguntou o que havia de segurança.
A resposta expôs um furo: **a rota não olhava qual arquivo**. O guard checava só
"esta sessão marcou alguma rota?" e liberava `src/` inteiro. A separação era
convenção, não trava.

Ele então perguntou se branch não seria mais seguro. Seria, com uma ressalva que
importa: **branch sozinha é pior que nada**, porque o git guarda uma cópia dos
arquivos por pasta e `git checkout` reescreve o disco debaixo do outro agente. O
que isola é worktree.

Ficaram quatro peças, cada uma cobrindo o que a outra não cobre: rota reivindica
arquivo com 📁, oficina por agente (`cc oficina`), `branch-guard` contra comando
destrutivo, e **o quadro de rotas único na pasta principal** — achado minutos
depois de criar a primeira oficina, quando cada worktree apareceu com sua própria
cópia do quadro, vinda do último commit.

Sobre guardar tudo isso como dado, que foi a pergunta dele: **derivar é melhor
que armazenar.** `git worktree list` sempre acerta; um JSON de "quem está onde"
mentiria no primeiro `remove` feito à mão.

### O merge, e o que estava errado no caminho

59 commits para o master, `e75c666`. A branch não tinha upstream e a referência
local do remoto estava velha — parecia faltar 10 commits. Um `fetch` antes de
mergear resolveu, em vez de acreditar no número.

### O CC-92 fechado sem proxy

O proxy ficaria como direção pelo risco (TLS, certificado, ponto único de falha
entre o CLI e a API). Mas `UserPromptSubmit` roda **antes** de o texto virar
prompt, na máquina dele: mesmo ponto de interceptação, sem nada do que torna o
proxy perigoso. Texto colado com CPF agora é recusado. Sobra a imagem, que
nenhum dos dois resolveria — o proxy veria bytes, não conteúdo.

### O defeito que ele achou usando

> "no telefone, ao tentar selecionar uma opção no menu o menu fica sumindo"

O `render()` do stream reescrevia `#tabs` a cada 2 segundos e destruía o
`<select>` aberto, levando o popup nativo junto. É a armadilha do textarea de
novo, e enganou porque o `<select>` parece seguro: é pequeno e não tem texto
digitado. **O que se perdia não era conteúdo, era estado do sistema operacional
— que nem aparece no DOM.**

O teste ficou melhor que o conserto: ele desliga a própria guarda e confere que
o defeito volta. Teste que só sabe dizer "hoje passa" não prova que pegaria a
regressão.

## O que sobrou, e por que

**Zero itens abertos e executáveis.** Restam dois, nenhum comigo:

- **CC-80**, decisão dele: o estudo está pronto com três opções medidas
- **CC-08**, um Mac

Fora do ROADMAP, esperando o Felipe: abrir `/hooks` (ou reiniciar) para os
hooks novos valerem, o deploy do Pierre, e o provider do fork em
`app_escritorio` ler `GET /api/escritorio`.

As 9 camadas restantes da Bancada precisam de Gitleaks, Trivy, Semgrep e
Nuclei — nenhum instalado, e instalar exige root.
